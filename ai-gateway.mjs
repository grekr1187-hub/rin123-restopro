...
async function authContext(req){const token=String(req.get("authorization")||"").replace(/^Bearer\\s+/i,"");if(!token)throw Object.assign(new Error("Требуется вход"),{status:401});const r=await q("SELECT s.id,s.role,s.restaurant_id FROM auth_sessions a JOIN staff s ON s.id=a.staff_id WHERE a.token=$1 AND a.expires_at>now() AND s.active=true",[token]);if(!r.rowCount)throw Object.assign(new Error("Сессия истекла"),{status:401});const role=/официант|waiter/i.test(String(r.rows[0].role))?"waiter":/администратор|admin/i.test(String(r.rows[0].role))?"admin":"owner";return {role,staffId:Number(r.rows[0].id),restaurantId:r.rows[0].restaurant_id};}
...
app.post("/api/ai/chat", async (req,res)=>{
  try {
    ...
    const ctx=await authContext(req); const restaurantId=ctx.restaurantId; const role=ctx.role; const staffId=ctx.staffId; const sessionId=String(req.body.sessionId||req.get("x-ai-session")||crypto.randomUUID());
